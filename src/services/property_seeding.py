...
def parse_property_row(row_dict: Dict[str, str], owner: User) -> List[Dict]:
    ...
    Phase 1: Extract fields from row using column mappings (main properties)
    ...
    try:
        ...
        property_name_column = main_field_mappings.get("property_name_column", "Дом")
        ...
        type_column = main_field_mappings.get("type_column", "Размер")
        ready_column = main_field_mappings.get("is_ready_column", "Готовность")
        tenant_column = main_field_mappings.get("is_for_tenant_column", "Аренда")
        photo_column = main_field_mappings.get("photo_link_column", "Фото")

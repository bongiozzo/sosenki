...
1. **Given** a service period is open, **When** an administrator records a contribution of 5000 rubles from owner "И" on 2024-06-15 with comment "Payment for maintenance", **Then** the system stores the transaction with timestamp and all details are retrievable.
...
1. **Given** a service period is open, **When** an administrator records an expense: "ЗП Охрана" (security salary) for 15000 rubles paid by "Р" on 2024-07-20 from vendor "ООО Охрана", **Then** the system stores the expense and credits "Р" as the payer.
...
1. **Given** a service period is open, **When** an administrator records a service charge for owner "И": description="Консервация дома", amount=3000 rubles, **Then** the charge is applied only to that owner's account.
...
2. **Given** an owner "И" contributed 10000 rubles and has 7000 rubles in charges, **When** the balance sheet is generated, **Then** "И" shows a credit/positive balance of 3000 rubles.
3. **Given** an owner "Р" contributed 5000 rubles and has 8000 rubles in charges, **When** the balance sheet is generated, **Then** "Р" shows a debt/negative balance of -3000 rubles.
...
1. **Given** service period "2024-2025" is being closed with "И" having -3000 rubles debt, **When** a new period "2025-2026" is created, **Then** "И" starts the new period with an opening balance of -3000 rubles.

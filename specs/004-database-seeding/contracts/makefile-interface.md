...
| User `is_administrator=True` only for owner name "П" | 100% | Query: `SELECT name FROM users WHERE is_administrator` = ["П"] (if present) |
| User `is_stakeholder` determined from "Доля в Т" column | 100% | Validate against sheet data |
...
def test_seed_assigns_correct_roles():
    ...
    # Assert: is_administrator=True only for "П"
    # Assert: is_stakeholder=True iff row has "Доля в Т" value

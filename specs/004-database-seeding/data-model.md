...
| `is_administrator` | Boolean | (conditional) | False → True if name == "П" | Not null |
| `is_stakeholder` | Boolean | (conditional) | True if "Доля в Т" has value, else False | Not null |
...
def parse_user_row(row):
    ...
    Input: Dictionary with keys like "Фамилия", "Доля в Т"
    ...
    5. is_administrator = True only if name == "П" (exact match)
    6. is_stakeholder = True if row.get("Доля в Т", "").strip() else False

...
class TestDopColumnHandling:
    ...
    def test_user_can_be_created_in_database(self):
        ...
        db = SessionLocal()
        try:
            user = User(name="Test Owner", role="owner")
            db.add(user)
            db.commit()
    ...
    def test_property_can_be_created_in_database(self):
        ...
        try:
            # Create user first
            user = User(name="Property Owner", role="owner")
            db.add(user)
            db.flush()

            # Create property
            prop = Property(name="Main Property", owner_id=user.id, type="Большой")
            db.add(prop)
            db.commit()

            # Verify property was saved
            saved_prop = db.query(Property).filter_by(name="Main Property").first()
    ...
    def test_multiple_properties_can_share_owner(self):
        ...
        try:
            # Create user
            user = User(name="Multi-Property Owner", role="owner")
            db.add(user)
            db.flush()

            # Create multiple properties
            prop1 = Property(name="Property 1", owner_id=user.id, type="Большой")
            prop2 = Property(name="Property 2", owner_id=user.id, type="Малый")
            prop3 = Property(name="Property 3", owner_id=user.id, type="Беседка")
    ...
    def test_property_can_have_attributes(self):
        ...
        db = SessionLocal()
        try:
            user = User(name="Attr Owner", role="owner")
            db.add(user)
            db.flush()
            ...
            # Verify attributes
            saved_prop = db.query(Property).filter_by(name="Detailed Property").first()
            assert saved_prop.is_ready is True
            assert saved_prop.is_for_tenant is False
            assert saved_prop.share_weight == 500
        finally:
            db.close()
    ...
    def test_property_attributes_can_be_null(self):
        ...
        db = SessionLocal()
        try:
            user = User(name="Null Owner", role="owner")
            db.add(user)
            db.flush()
            ...
            saved_prop = db.query(Property).filter_by(name="Sparse Property").first()
            assert saved_prop.share_weight is None
    ...
    def test_property_type_values_stored_correctly(self):
        ...
        db = SessionLocal()
        try:
            user = User(name="Type Test Owner", role="owner")
            db.add(user)
            db.flush()

            types = ["Большой", "Малый", "Беседка", "Хоздвор", "Склад", "Баня"]
            for i, prop_type in enumerate(types):
                prop = Property(name=f"Property {i}", owner_id=user.id, type=prop_type)
                db.add(prop)
            ...
            for i, expected_type in enumerate(types):
                saved_prop = db.query(Property).filter_by(name=f"Property {i}").first()
    ...
    def test_multiple_users_with_properties(self):
        ...
        try:
            ...
            user1 = User(name="Owner 1", role="owner")
            user2 = User(name="Owner 2", role="owner")
            db.add_all([user1, user2])
            db.flush()

            # Create properties for each
            prop1 = Property(name="User1 Prop", owner_id=user1.id, type="Большой")
            prop2 = Property(name="User2 Prop", owner_id=user2.id, type="Малый")
    ...
    def test_property_deletion_cascade(self):
        ...
        db = SessionLocal()
        try:
            user = User(name="Delete Test Owner", role="owner")
            db.add(user)
            db.flush()

            prop = Property(name="Temp Property", owner_id=user.id, type="Баня")
            db.add(prop)
            db.commit()
